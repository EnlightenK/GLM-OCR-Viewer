...
const PdfViewerCanvasOnly: React.FC<PdfViewerProps> = ({
    ...
    const [defaultPageHeight, setDefaultPageHeight] = useState<number>(VIRTUAL_CONFIG.PLACEHOLDER_HEIGHT);
    ...
    const getPageOffset = useCallback((pageNumber: number): number => {
        ...
        for (let i = 1; i < pageNumber; i++) {
            const pageHeight = (pageHeights[i] || defaultPageHeight) * scale;
            offset += pageHeight + 20; // 缩放后的页面高度 + mb-5 间距
        }
        return offset;
    }, [pageHeights, scale, defaultPageHeight]);
    ...
    const onDocumentLoadSuccess = async (pdfDocument: { numPages: number; getPage: (n: number) => Promise<any> }) => {
        ...
        // Read actual first-page dimensions from the PDF (fallback to A4)
        let pageWidth = 595;
        let pageHeight = 842;
        try {
            ...
            setDefaultPageHeight(pageHeight);
        } catch {
            // keep A4 defaults
        }
    ...
    const totalHeight = useMemo(() => {
        let height = 0;
        for (let i = 1; i <= numPages; i++) {
            height += (pageHeights[i] || defaultPageHeight) * scale;
        }
        // 添加页面之间的间距（最后一个页面不需要底部间距）
        height += (numPages > 0 ? (numPages - 1) * 20 : 0);
        return height;
    }, [numPages, pageHeights, scale, defaultPageHeight]);
    ...
    }, [viewportHeight, numPages, pageHeights, scale, getPageOffset, defaultPageHeight]);
    ...
    const renderPage = (pageNumber: number) => {
        const height = pageHeights[pageNumber] || defaultPageHeight;
